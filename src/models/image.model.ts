  ...
  getStats(): {
    ...
    const stats = db.prepare(`
      SELECT
        ...
        COALESCE(AVG(CASE WHEN status = 'completed' THEN overall_score ELSE NULL END), 0) as avgScore
      FROM images
    `).get() as any;

    // Ensure null avgScore becomes 0 for empty DB
    if (stats.avgScore === null) stats.avgScore = 0;
    return stats;
  },
};
